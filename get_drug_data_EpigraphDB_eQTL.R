...
#Load in the data with significant multi-SNP eQTL MR
table_eqtl <- read.delim("merged_multi_significant.txt", sep="\t", header=T, stringsAsFactors=F, row.names=NULL, quote="")
...
write.table(clingov_vs_ot_st7, "clingov_vs_ot_multieqtl.tsv", sep="\t", quote=F, row.names=F)
...
write.table(clingov_vs_ot_st7_slim, "clingov_vs_ot_multieqtl_slim.tsv", sep="\t", quote=F, row.names=F)
...
write.table(clingov_vs_cpic_st7, "clingov_vs_cpic_multieqtl.tsv", sep="\t", quote=F, row.names=F)

#A slimmed table with just unique set of drug name, gene target name and condition
clingov_vs_cpic_st7_slim <- clingov_vs_cpic_st7 %>% select(drug, all_cond, g1.ensembl_id, g1.name) %>% distinct() %>% arrange(g1.name)
write.table(clingov_vs_cpic_st7_slim, "clingov_vs_cpic_multieqtl_slim.tsv", sep="\t", quote=F, row.names=F)

#Read in matches between eQTLs and ClinicalTrials.Gov
matches <- read.delim("multieqtl_selected.txt", sep="\t", header=T, stringsAsFactors=F, row.names=NULL, quote="")
...
write.table(clingov_merged_table7, "clingov_multieqtl_merged.txt", sep="\t", quote=F, row.names=F)
