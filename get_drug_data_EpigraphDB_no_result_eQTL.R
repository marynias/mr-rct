...
clingov <- read.delim("clingov_noresult.tsv", sep="\t", header=T, stringsAsFactors=F, row.names=NULL, quote="")
...
write.table(clingov_vs_ot_st7, "clingov_vs_ot_multieqtl_noresult.tsv", sep="\t", quote=F, row.names=F)
...
write.table(clingov_vs_ot_st7_slim, "clingov_vs_ot_multieqtl_slim_noresult.tsv", sep="\t", quote=F, row.names=F)
...
write.table(clingov_vs_cpic_st7, "clingov_vs_cpic_multieqtl_noresult.tsv", sep="\t", quote=F, row.names=F)

#A slimmed table with just unique set of drug name, gene target name and condition
clingov_vs_cpic_st7_slim <- clingov_vs_cpic_st7 %>% select(drug, all_cond, g1.ensembl_id, g1.name) %>% distinct() %>% arrange(g1.name)
write.table(clingov_vs_cpic_st7_slim, "clingov_vs_cpic_multieqtl_slim_noresult.tsv", sep="\t", quote=F, row.names=F)
